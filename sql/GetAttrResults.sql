SELECT 
	CASE 
		WHEN afrd.DefectCount = 0 THEN 1
		ELSE -1
	END AS [Result]
FROM MeasurLink7.dbo.Run r
LEFT OUTER JOIN MeasurLink7.dbo.Routine rt ON r.RoutineID  = rt.RoutineID 
LEFT OUTER JOIN MeasurLink7.dbo.FeatureRun fr ON r.RunID = fr.RunID 
LEFT OUTER JOIN MeasurLink7.dbo.Feature f ON f.FeatureID = fr.FeatureID 
LEFT OUTER JOIN MeasurLink7.dbo.AttFeatureRunData afrd ON fr.RunID  = afrd.RunID AND fr.FeatureID = afrd.FeatureID 
WHERE r.RunName = ? AND rt.RoutineName = ? AND f.FeatureName = ? AND afrd.DefectCount IS NOT NULL;